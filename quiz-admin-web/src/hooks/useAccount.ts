...
export const useAccount = () => {
    ...
    const getAccountInfo = () => {
        return Object.keys(accountInfo ?? {}).length ? accountInfo : defaultAccountStore
    }

    const hasLogin = () => {
        const accountInfo = getAccountInfo()
        const localAccount = getLocalAccount()
        const checkNotEmpty = (u: AccountInfo) => CheckUtils.isNotEmpty(u.token) && CheckUtils.isNotEmpty(u.uid)
        return checkNotEmpty(accountInfo) && checkNotEmpty(localAccount)
    }

    const setAccountInfo = (accountInfo: AccountInfo | undefined) => {
        const newAccountInfo = { ...(accountInfo ?? {}), uid: String(accountInfo?.uid ?? '') }
