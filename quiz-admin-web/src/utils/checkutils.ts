export default class CheckUtils {
    ...
    static isYouTubeChannelUrl(url: string): boolean {
        const youtubeChannelUrlPattern = /^(?:https?:\/\/)?(?:www\.)?youtube\.com\/@([A-Za-z0-9_-]+)/
        const youtubeShortenedUrlPattern = /^(?:https?:\/\/)?(?:www\.)?youtu\.be\/@([A-Za-z0-9_-]+)/
        return youtubeChannelUrlPattern.test(url) || youtubeShortenedUrlPattern.test(url)
    }
    ...
    static isYouTubeVideoUrl(url: string): boolean {
        ...
        const youtubeShortenedUrlPattern = /^(?:https?:\/\/)?(?:www\.)?youtu\.be\/([A-Za-z0-9_-]+)$/
        return youtubeVideoUrlPattern.test(url) || youtubeShortenedUrlPattern.test(url)
    }
}
