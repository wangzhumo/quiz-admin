...
import { BUS_TOKEN_EXPIRE_KEY, BUS_TOKEN_INVALID_KEY, RequestData, ResponsePromiseData } from './types'
...
class BaseRequest {
    ...
    protected constructor(baseUrl: string) {
        ...
        this.instanceRequest.interceptors.request.use(
            config => {
                ...
                const headers: any = { ...config.headers, authorization: (accountInfo as any).token ?? '' }
    ...
    protected readonly requestBefore = (url: string) => {
        ...
        const filterUrl = TOKEN_CHECK_WHITE_LIST.filter(value => value.includes(url)) ?? []
    ...
    protected readonly baseGet = (url: string, config: AxiosRequestConfig = {}) => {
    ...
    protected readonly basePost = <D>(url: string, data: RequestData<D>, config: AxiosRequestConfig) => {
        return this.baseRequest({
            ...config,
            data,
            method: 'POST',
    ...
    protected readonly baseUpload = <D>(url: string, data: RequestData<D>, config: AxiosRequestConfig) => {
        return this.baseRequest({
            ...config,
            data,
            method: 'POST',
    ...
    protected readonly baseDelete = <D>(url: string, data: RequestData<D>, config: AxiosRequestConfig) => {
        return this.baseRequest({
            ...config,
            data,
            method: 'DELETE',
...
export default class Request extends BaseRequest {
    ...
    readonly simpleGet = (url: string, params: any, config: AxiosRequestConfig = {}) => {
        let host = url
    ...
    readonly post = async <T>(url: string, data: RequestData<T>, config: AxiosRequestConfig = {}) => {
    ...
    readonly upload = async <T>(url: string, data: RequestData<T>, config: AxiosRequestConfig = {}) => {
    ...
    readonly delete = async <T>(url: string, data: RequestData<T>, config: AxiosRequestConfig = {}) => {
